...
resource "bigip_gtm_server" "generic_with_vs" {
  ...
  # Define virtual servers for generic host
  virtual_servers {
    name        = "vs_http"
...
* `virtual_servers` - (Optional) List of virtual servers for the GTM server. This is particularly useful for generic-host servers where virtual server discovery is not available. Each virtual_servers block supports:
  - `name` - (Required) Name of the virtual server
  - `destination` - (Required) Destination address in format `<ip>:<port>` (e.g., `192.168.1.100:80`)
  - `enabled` - (Optional) Enable the virtual server. Default is `true`
  - `translation_address` - (Optional, Computed) Translation address for NAT scenarios. Default is `none`
  - `translation_port` - (Optional, Computed) Translation port for NAT scenarios. Default is `0`
  - `monitor` - (Optional, Computed) Monitor assigned to check virtual server health
  - `limit_max_bps` - (Optional) Maximum bits per second for this virtual server
  - `limit_max_bps_status` - (Optional) Enable/disable the bps limit
  - `limit_max_connections` - (Optional) Maximum concurrent connections
  - `limit_max_connections_status` - (Optional) Enable/disable the connections limit
  - `limit_max_pps` - (Optional) Maximum packets per second
  - `limit_max_pps_status` - (Optional) Enable/disable the pps limit
