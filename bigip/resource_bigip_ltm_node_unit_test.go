...
func TestAccBigipLtmNodeUnitInvalid(t *testing.T) {
...
func TestAccBigipLtmNodeUnitCreate(t *testing.T) {
	resourceName := "/Common/test-node"
	address := "10.10.10.10"
	setup()
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config:             testBigipLtmNodeModify(resourceName, server.URL, address),
...
func TestAccBigipLtmNodeUnitCreateFqdn(t *testing.T) {
	resourceName := "/Common/test-node"
	address := "f5.com"
	setup()
	mux.HandleFunc("mgmt/shared/authn/login", func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("/mgmt/tm/ltm/node/~Common~test-node", func(w http.ResponseWriter, r *http.Request) {
		_, _ = fmt.Fprintf(w, `{"name":"%s","address":"%s","monitor":"/Common/icmp","fqdn": {"tmName": "f5.com"}}`, resourceName, address)
	})
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testBigipLtmNodeCreateFqdn(resourceName, server.URL, address),
			},
		},
	})
}
...
func testBigipLtmNodeModify(resourceName string, url string, address string) string {
	return fmt.Sprintf(`
resource "bigip_ltm_node" "test-node" {
  name    = "%s"
  address = "%s"
...
func testBigipLtmNodeCreateFqdn(resourceName, url, address string) string {
	return fmt.Sprintf(`
resource "bigip_ltm_node" "test-node" {
  name    = "%s"
  address = "%s"
